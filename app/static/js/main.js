...
document.addEventListener('DOMContentLoaded', function() {
    ...
    passwordInputs.forEach(input => {
        // Skip password fields that already have strength meters
        // Check if this is a field in the auth forms (change password, reset password)
        const isAuthPasswordField = input.id === 'current_password' ||
                                   input.id === 'new_password' ||
                                   input.id === 'confirm_password' ||
                                   input.id === 'password';

        // Skip if it's an auth password field or if it already has a strength meter
        if (isAuthPasswordField || input.parentNode.querySelector('.password-strength-meter')) {
            return;
        }
